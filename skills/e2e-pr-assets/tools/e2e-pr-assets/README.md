...
bash tools/e2e-pr-assets/bin/e2e-run-script tools/e2e-pr-assets/test/popup-overlay.scenario.mjs --repo . --label popup-overlay-test --record
```
...
- Recorded action overlays remain visible for 1.2 seconds, and the next wrapped interaction waits for the current overlay to disappear so commands such as `Cmd/Ctrl + Click` stay readable without scenario-specific sleeps.
- `video?.waitForPage(newTab)` also waits for the source-page overlay before stitching the popup/new-tab beat. The destination starts clean by default; pass `{ replayLastOverlay: true }` only when the previous command should intentionally remain visible there.
