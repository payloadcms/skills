...
- Helper commands installed by the bundle:
  ...
  - `e2e-attach-pr`
- For repeat use, prefer the top-level config helper over retyping env vars:
  ...
  - The config file includes an `Available keys` comment section with descriptions/defaults and a `Configured keys` section with only your explicit overrides.
  - `e2e-pr-assets --show-config` prints the config path first, then the config file contents.
...
10. Upload through GitHub's attachment UI to get `https://github.com/user-attachments/assets/...` URLs:
   ...
   - Prefer storing repeat-use defaults with `e2e-pr-assets --configure ...`; use env vars only for one-off overrides.
...
- Reusable defaults can live in the config file managed by `e2e-pr-assets --configure`, and `e2e-pr-assets --show-config` remains the readable place to inspect the available/configured sections.
